...
public:
  ...
  Q_INVOKABLE QVariantMap run(const QString &id,
                              const QVariantMap &args);

  //Q_INVOKABLE QList<QString> functions() const { return _functions.keys(); }
  Q_INVOKABLE QVariantList functions() const { return QVariant::fromValue(QStringList(_functions.keys())).toList(); }
  Q_INVOKABLE bool hasFunction(const QString &id) const { return _functions.contains(id); }
