...
namespace core {
  ...
  void insertReply(reply source,unordered_map &_unordered_map){
    if ( _unordered_map.empty()) _unordered_map.emplace(source.word,middleResult(1,source.rank));
    else {
    for(int i=0 ; i < _unordered_map.size(); i++){
        for (auto &wordMap : _unordered_map ){
          if(wordMap.first == source.word ){
            wordMap.second.occurences++;
            wordMap.second.sum+=source.rank;
          }
          else {
            _unordered_map.emplace(source.word,middleResult(1,source.rank));
          }
        }
      }
    }
  ...
  results Calcul(std::vector<stringVec> form){
    ...
    for (auto &wordMap : _unordered_map ){
      ...
      frequency = ( double(wordMap.second.sum) / totalOfOccurence);
      _result.push_back(result(wordMap.first,averageRank,frequency));
      averageRank = 0;
    }
    return _result;
